 ...
 */
static option_t eap_option_list[] = { { "eap-restart", o_int,
		&eap_states[0].es_server.ea_timeout,
		"Set retransmit timeout for EAP Requests (server)" }, { "eap-max-sreq",
		o_int, &eap_states[0].es_server.ea_maxrequests,
		"Set max number of EAP Requests sent (server)" }, { "eap-timeout",
		o_int, &eap_states[0].es_client.ea_timeout,
		"Set time limit for peer EAP authentication" }, { "eap-max-rreq",
		o_int, &eap_states[0].es_client.ea_maxrequests,
		"Set max number of EAP Requests allows (client)" }, { "eap-interval",
		o_int, &eap_states[0].es_rechallenge,
...
struct protent eap_protent = { PPP_EAP, /* protocol number */
...
static void eap_send_success(esp)
	...
	auth_peer_success(esp->es_unit, PPP_EAP, 0, esp->es_server.ea_peer,
...
static void eap_send_request(esp)
	...
	if (esp->es_server.ea_state < eapIdentify && esp->es_server.ea_state
			!= eapInitial) {
	...
	if (esp->es_server.ea_maxrequests > 0 && esp->es_server.ea_requests
			>= esp->es_server.ea_maxrequests) {
	...
	case eapMD5Chall:
		 ...
		 */
		challen = (drand48() * (MAX_CHALLENGE_LENGTH - MIN_CHALLENGE_LENGTH))
...
void eap_authpeer(unit, localname)
	...
	/* Lower layer up yet? */
	if (esp->es_server.ea_state == eapInitial || esp->es_server.ea_state
			== eapPending) {
...
static void eap_rechallenge(arg)
	...
	if (esp->es_server.ea_state != eapOpen && esp->es_server.ea_state
			!= eapSRP4)
		return;
...
static void srp_lwrechallenge(arg)
	...
	if (esp->es_server.ea_state != eapOpen || esp->es_server.ea_type
			!= EAPT_SRP)
		return;
...
static void eap_send_response(esp, id, typenum, str, lenstr)
	eap_state *esp;u_char id;u_char typenum;u_char *str;int lenstr; {
	u_char *outp;
	int msglen;
...
static void eap_chap_response(esp, id, hash, name, namelen)
	eap_state *esp;u_char id;u_char *hash;char *name;int namelen; {
	u_char *outp;
	int msglen;
...
static void eap_send_nak(esp, id, type)
	eap_state *esp;u_char id;u_char type; {
	u_char *outp;
	int msglen;
...
static void eap_request(esp, inp, id, len)
	...
	if (esp->es_client.ea_maxrequests != 0 && esp->es_client.ea_requests
			> esp->es_client.ea_maxrequests) {
	...
	case EAPT_MD5CHAP:
		 ...
		 * the specified host.
		 */
		if (!get_secret(esp->es_unit, esp->es_client.ea_name, rhostname,
				secret, &secret_len, 0)) {
...
static void eap_response(esp, inp, id, len)
	...
	case EAPT_IDENTITY:
		...
		if (esp->es_server.ea_peer != NULL && esp->es_server.ea_peer
				!= remote_name)
			free(esp->es_server.ea_peer);
	...
	case EAPT_MD5CHAP:
		 ...
		 * host.
		 */
		if (!get_secret(esp->es_unit, rhostname, esp->es_server.ea_name,
				secret, &secret_len, 1)) {
	...
	if (esp->es_server.ea_state != eapBadAuth && esp->es_server.ea_state
			!= eapOpen) {
		esp->es_server.ea_id++;
		eap_send_request(esp);
	}
 ...
 */
static char *eap_codenames[] = { "Request", "Response", "Success", "Failure" };

static char *eap_typenames[] = { "Identity", "Notification", "Nak",
		"MD5-Challenge", "OTP", "Generic-Token", NULL, NULL, "RSA", "DSS",
		"KEA", "KEA-Validate", "TLS", "Defender", "Windows 2000", "Arcot",
		"Cisco", "Nokia", "SRP" };
